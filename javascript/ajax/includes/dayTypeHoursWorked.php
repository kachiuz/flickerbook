...
for($i=0; $i<12; $i++){
$querySelectDay = "SELECT COUNT(dayType) FROM day_indexes WHERE user_id = '$user_id' AND dayType = '$i' AND taxPeriodNr >= '$firstTaxPeriodOfYear' AND taxPeriodNr <= '$taxPeriodNumber'"; 
...
$querySelectTotalHours = "SELECT SUM(hours_worked) FROM day_indexes WHERE user_id = '$user_id' AND dayType = '1' AND taxPeriodNr >= '$firstTaxPeriodOfYear' AND taxPeriodNr <= '$taxPeriodNumber'"; 
